  ...
  const advancePhase = useCallback(() => {
    ...
    if (phase === 'FOCUS') {
      ...
      if (cycle >= settings.totalCycles) {
        // All cycles done — go to long break
        resetTimerState();
        setPhase('LONG_BREAK');
        setRunning(true);
        startTimeRef.current = Date.now();
    ...
    <div className="flex flex-col items-center gap-6">
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className={cn('text-xs font-semibold tracking-widest mt-2', phaseGlow)}>
            {phaseLabel}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          className={cn(
            ...
            running && 'border-accent-mint/40 shadow-[0_0_16px_rgba(74,222,128,0.15)]',
          )}
        >
          {running ? (
            <svg className="w-6 h-6 text-accent-mint" fill="currentColor" viewBox="0 0 24 24">
        ...
        <button
          onClick={handleSkip}
          disabled={phase === 'IDLE'}
          className="w-12 h-12 rounded-2xl flex items-center justify-center bg-white/[0.04] backdrop-blur-xl border border-white/[0.06] text-text-muted hover:text-text-primary transition-colors disabled:opacity-30"
        >
        ...
        <button
          ...
          className={cn(
            'w-12 h-12 rounded-2xl flex items-center justify-center bg-white/[0.04] backdrop-blur-xl border border-white/[0.06] text-text-muted hover:text-text-primary transition-colors',
